...
################################################################################
# Default configurations. They can be overriden by boot.scr                    #
################################################################################
...
################################################################################
setenv set-hostname 'set hostname iptft-${ethaddr}'
...
################################################################################
setenv set-ip-static 'setenv ipargs ip=${ipaddr}:${serverip}:${gatewayip}:${netmask}:${hostname}:eth0:off'
...
setenv nfs-init 'if test true = true; then \
    ...
    dhcp; \
    set nfsroot /opt/exports/${tftp_path}/../rootfs; \
    fi'
...
setenv bootcmd 'if test true = true ; then \
    ...
    echo Initialize rootfs media ...; \
    run rootfs-init; \
    echo Loading user boot.scr configurations ...; \
    run load-user; \
    ...
    run set-console; \
    echo Loading update flag from updat.scr ...; 
    run load-update; \
   ...
   run set-bootargs ; \
   if run load-kernel ; then \
       ; \
   else \
       echo Reading kernel image from rootfs ${rootfsmedia}:${rootfstype} has failed; \
